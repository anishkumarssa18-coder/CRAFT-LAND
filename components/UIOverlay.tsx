...
import { Loader2, Coins, Users, Calendar, Newspaper, Target, X, Hammer, Trees, Home, Store, Factory, Construction } from 'lucide-react';
...
const ToolButton: React.FC<{
  ...
  const getIcon = () => {
    switch (type) {
      case BuildingType.None: return <X className="w-5 h-5 md:w-6 md:h-6 text-red-500" />;
      case BuildingType.Road: return <Construction className="w-5 h-5 md:w-6 md:h-6 text-gray-400" />;
      case BuildingType.Residential: return <Home className="w-5 h-5 md:w-6 md:h-6 text-red-400" />;
      case BuildingType.Commercial: return <Store className="w-5 h-5 md:w-6 md:h-6 text-blue-400" />;
      case BuildingType.Industrial: return <Factory className="w-5 h-5 md:w-6 md:h-6 text-yellow-500" />;
      case BuildingType.Park: return <Trees className="w-5 h-5 md:w-6 md:h-6 text-green-400" />;
      default: return null;
    }
  };
  ...
  return (
    <motion.button
      whileHover={!isBulldoze && !canAfford ? {} : { scale: 1.1, y: -4 }}
      whileTap={!isBulldoze && !canAfford ? {} : { scale: 0.95 }}
      onClick={onClick}
      ...
      className={`
        ...
        ${isSelected ? 'border-white bg-white/20 z-10' : 'border-gray-600 bg-gray-900/80 hover:bg-gray-800'}
        ...
        <div className={`w-full md:w-80 bg-indigo-900/90 text-white rounded-xl border-2 border-indigo-500/50 shadow-[0_0_20px_rgba(99,102,241,0.4)] backdrop-blur-md overflow-hidden transition-all ${!aiEnabled ? 'opacity-80 grayscale-[0.5]' : ''}`}>
          ...
          <div className="p-3 md:p-4">
            {aiEnabled ? (
              currentGoal ? (
                <>
                  <p className="text-xs md:text-sm font-medium text-indigo-100 mb-2 md:mb-3 leading-tight drop-shadow font-sans">"{currentGoal.description}"</p>
                  ...
                  <div className="flex justify-between items-center mt-1 md:mt-2 bg-indigo-950/60 p-1.5 md:p-2 rounded-lg border border-indigo-700/50">
                    <div className="text-[10px] md:text-xs text-gray-300 flex items-center gap-1">
                ...
                <div className="text-xs md:text-sm text-gray-400 py-2 italic flex items-center gap-2">
                  <Loader2 className="animate-spin h-3 w-3 md:h-4 md:w-4 text-indigo-400" />
                  Analyzing city data...
                </div>
              )
            ) : (
