      ...
      {(() => {
        switch (type) {
            ...
            if (variant < 20) {
              ...
              return (
                <>
                  ...
                  <WindowBlock position={[0.25, 0.35, 0.41]} scale={[0.2, 0.2, 0.05]} />
                </>
              );
            } else if (variant < 60) {
              // Apartment Block (Tall)
              return (
                <>
                  <mesh {...commonProps} material={mainMat} geometry={boxGeo} position={[0, 0.7, 0]} scale={[0.8, 1.4, 0.8]} />
                  <mesh {...commonProps} material={roofMat} geometry={boxGeo} position={[0, 1.45, 0]} scale={[0.85, 0.1, 0.85]} />
                  {[0, 1, 2, 3].map(i => (
                    <React.Fragment key={i}>
                      <WindowBlock position={[-0.2, 0.3 + i * 0.3, 0.41]} scale={[0.2, 0.15, 0.05]} />
                      <WindowBlock position={[0.2, 0.3 + i * 0.3, 0.41]} scale={[0.2, 0.15, 0.05]} />
                    </React.Fragment>
                  ))}
                </>
              ...
              return (
                <>
                  ...
                  <mesh {...commonProps} material={mainMat} geometry={boxGeo} position={[0.2, 0.3, 0.2]} scale={[0.4, 0.6, 0.5]} />
                  <mesh {...commonProps} material={roofMat} geometry={boxGeo} position={[-0.2, 0.65, 0]} scale={[0.55, 0.1, 0.95]} />
                  <mesh {...commonProps} material={roofMat} geometry={boxGeo} position={[0.2, 0.65, 0.2]} scale={[0.45, 0.1, 0.55]} />
                  <WindowBlock position={[-0.2, 0.35, 0.46]} scale={[0.3, 0.2, 0.05]} />
                </>
              );
            } else {
              // Townhouse
              ...
              return (
                <>
                  ...
                  {Array.from({ length: Math.floor(height * 4) }).map((_, i) => (
                    <WindowBlock key={i} position={[0, 0.2 + i * 0.25, 0]} scale={[0.72, 0.1, 0.72]} />
                  ))}
                  <mesh {...commonProps} material={accentMat} geometry={boxGeo} position={[0, height + 0.1, 0]} scale={[0.5, 0.2, 0.5]} />
                </>
              );
            } else if (variant < 55) {
              // Modern Office (Cylinder/Circular)
              const height = 1.5 + hash * 1.0;
              return (
                <>
                  <mesh {...commonProps} material={mainMat} geometry={cylinderGeo} position={[0, height/2, 0]} scale={[0.4, height, 0.4]} />
                  {Array.from({ length: Math.floor(height * 3) }).map((_, i) => (
                    <mesh key={i} geometry={cylinderGeo} position={[0, 0.3 + i * 0.4, 0]} scale={[0.41, 0.15, 0.41]} material={new THREE.MeshStandardMaterial({ color: '#bfdbfe', emissive: '#bfdbfe', emissiveIntensity: 0.1 })} />
                  ))}
                  <mesh {...commonProps} material={accentMat} geometry={sphereGeo} position={[0, height, 0]} scale={[0.35, 0.2, 0.35]} />
                </>
              );
            } else if (variant < 80) {
              // Shop
            ...
            if (variant < 40) {
              ...
              return (
                <>
                  <mesh {...commonProps} material={mainMat} geometry={boxGeo} position={[0, 0.4, 0]} scale={[0.9, 0.8, 0.8]} />
                  {[ -0.3, 0, 0.3 ].map((z, i) => (
                    <mesh key={i} {...commonProps} material={roofMat} geometry={boxGeo} position={[0, 0.9, z]} scale={[0.9, 0.2, 0.2]} rotation={[Math.PI/4, 0, 0]} />
                  ))}
              ...
              return (
                <>
                  <mesh {...commonProps} material={mainMat} geometry={boxGeo} position={[-0.2, 0.3, 0]} scale={[0.5, 0.6, 0.9]} />
                  <mesh {...commonProps} material={new THREE.MeshStandardMaterial({color: '#94a3b8'})} geometry={cylinderGeo} position={[0.25, 0.6, -0.2]} scale={[0.25, 1.2, 0.25]} />
                  <mesh {...commonProps} material={new THREE.MeshStandardMaterial({color: '#94a3b8'})} geometry={cylinderGeo} position={[0.25, 0.6, 0.2]} scale={[0.25, 1.2, 0.25]} />
                  <mesh {...commonProps} material={accentMat} geometry={sphereGeo} position={[0.25, 1.2, -0.2]} scale={[0.25, 0.1, 0.25]} />
                  <mesh {...commonProps} material={accentMat} geometry={sphereGeo} position={[0.25, 1.2, 0.2]} scale={[0.25, 0.1, 0.25]} />
                </>
              );
            } else {
              ...
              return (
                <>
                  <mesh {...commonProps} material={mainMat} geometry={boxGeo} position={[0, 0.4, 0]} scale={[1, 0.8, 1]} />
                  <mesh {...commonProps} material={accentMat} geometry={boxGeo} position={[0, 0.85, 0]} scale={[0.4, 0.1, 0.4]} />
                  <mesh {...commonProps} material={new THREE.MeshStandardMaterial({color: '#475569'})} geometry={boxGeo} position={[0.51, 0.3, 0]} scale={[0.02, 0.4, 0.5]} />
                  <mesh {...commonProps} material={new THREE.MeshStandardMaterial({color: '#475569'})} geometry={boxGeo} position={[-0.51, 0.3, 0]} scale={[0.02, 0.4, 0.5]} />
                </>
              );
            }
          ...
          case BuildingType.Park:
            ...
            const positions = [[-0.25, -0.25], [0.25, 0.25], [-0.25, 0.25], [0.25, -0.25], [0, 0]];
            ...
            return (
              <group position={[0, -yOffset - 0.29, 0]}>
                ...
                {variant < 40 ? (
                    ...
                    // Statue / Monument variant
                    <group position={[0, 0.05, 0]}>
                      <mesh material={accentMat} geometry={boxGeo} scale={[0.3, 0.4, 0.3]} position={[0, 0.2, 0]} castShadow receiveShadow />
                      <mesh material={new THREE.MeshStandardMaterial({ color: '#fcd34d', metalness: 0.8, roughness: 0.2 })} geometry={sphereGeo} position={[0, 0.5, 0]} scale={0.15} castShadow />
                    </group>
                ) : null}

                {Array.from({length: treeCount}).map((_, i) => {
                    const pos = positions[i % positions.length];
                    // Skip center if something else is there
                    if ((variant < 70) && pos[0] === 0 && pos[1] === 0) return null;
                    
                    const scale = 0.4 + getHash(x+i, y-i) * 0.6;
